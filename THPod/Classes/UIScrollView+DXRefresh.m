...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.acv.center = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) - 5);
        self.acv.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        UILabel * footer = [[UILabel alloc] initWithFrame:CGRectMake(0, 28, [[UIScreen mainScreen] bounds].size.width , 20)];
        footer.font = [UIFont fontWithName:@"Helvetica" size:11];
        footer.textAlignment = NSTextAlignmentCenter;
        footer.textColor = [UIColor blackColor];
//        footer.text = @"Đang tải dữ liệu";
...
@property (nonatomic, strong) ODRefreshControl *header;
...
//    }
    ...
    [refreshControl addTarget:self action:@selector(_headerAction) forControlEvents:UIControlEventValueChanged];
...
//    }
//    refresh.attributedTitle = [[NSAttributedString alloc] initWithString:@"Đang tải dữ liệu"];
//    refresh.transform = CGAffineTransformMakeScale(0.9, 0.9);
...
//        }
//    }
    if ([self.header isKindOfClass:[ODRefreshControl class]])
    {
...
//    }
    if ([self.header isKindOfClass:[ODRefreshControl class]])
    {
        ODRefreshControl *refresh = (ODRefreshControl *)self.header;
        CGFloat contentOffsetY = _kRefreshControlHeight - self.contentInset.top;
        [self setContentOffset:CGPointMake(0, contentOffsetY) animated:YES];
        [refresh beginRefreshing];
    }
...
- (void)addFooterWithTarget:(id)target action:(SEL)action withIndicatorColor:(UIColor *)color
{
    ...
    self.footer.acv.transform = CGAffineTransformMakeScale(1.1, 1.1);
